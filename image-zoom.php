	...
	*/
	...
	function _modify_content_callback($matches) {
		...
		if (preg_match($pattern_img, $matches[0])) {
			return preg_replace_callback($pattern_img, array($this, "_stripslashes_callback"), $matches[0]);
		}
	...
	function _stripslashes_callback($m) {
		// comme d'habitude : $matches[0] represente la valeur totale
  		// $matches[1] represente la première parenthèse capturante
  		
  		return ("<a".$m[2]."href=\"".$m[3]."".$m[4]."\" class=\"gallery_colorbox\"".$m[5].">".$m[6]."<img".$m[7]."src=\"".$m[8]."".$m[9]."\" ".$m[10].">".$m[11]."</a>");
	}
